...
import { useClassDetails, useSubmitBulkAttendance } from "../../hooks/useTeacherData";
...
export default function ClassAttendanceScreen() {
    ...
    return (
      <ScreenWrapper>
        <View className="flex-1 items-center justify-center p-6">
          <Text className="text-on-surface-variant font-bold text-lg">Class not found</Text>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert("Error", error.response?.data?.message || "Failed to submit attendance. Please try again.");
    }
  };
    ...
    <ScreenWrapper>
      ...
      <ScrollView showsVerticalScrollIndicator={false} className="flex-1 pb-32">
        ...
        <View className="mb-6">
          ...
          <Text className="text-on-surface-variant text-sm mt-1">{dateDisplay}</Text>
        </View>
        ...
        <View className="flex-row gap-3 mb-6">
          ...
          >
            <Text className="text-xs font-bold text-success">Mark All Present</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => markAll("ABSENT")}
            className="flex-1 bg-error/10 py-3 rounded-xl items-center border border-error/20"
          >
            <Text className="text-xs font-bold text-error">Mark All Absent</Text>
          </TouchableOpacity>
        </View>
        ...
        <View className="gap-3">
          ...
          {students.length === 0 ? (
            <Card variant="dim" className="p-6 items-center">
              <Text className="text-on-surface-variant">No students enrolled in this class.</Text>
            </Card>
          ) : (
            students.map((enrollment: any) => {
              const student = enrollment.student;
              const user = student?.user;
              const fullName = user ? `${user.firstName} ${user.lastName}` : "Unknown Student";
              const rollNo = student?.rollNo || "N/A";
              const currentStatus = attendanceMap[student?.id] || "PRESENT";
              const avatar = user?.profileImage || "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSeJTiV7AMRW_xObVhIqXKza_MetiafhuqwnA&s";

              return (
                <Card key={student?.id} variant="dim" className="p-4 flex-row justify-between items-center border border-outline/50">
                  ...
                  <View className="flex-row bg-surface border border-outline rounded-xl p-1 gap-1">
                    ...
                    >
                      <Text className={`text-[10px] font-bold ${currentStatus === "PRESENT" ? "text-white" : "text-on-surface-variant"}`}>
                        P
                      </Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                    ...
                    >
                      <Text className={`text-[10px] font-bold ${currentStatus === "ABSENT" ? "text-white" : "text-on-surface-variant"}`}>
                        A
                      </Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                    ...
                    >
                      <Text className={`text-[10px] font-bold ${currentStatus === "LATE" ? "text-white" : "text-on-surface-variant"}`}>
                        L
                      </Text>
                    </TouchableOpacity>
                  </View>
      ...
      <View className="absolute bottom-6 left-4 right-4 bg-transparent">
        ...
        >
          ...
          <Text className="text-white font-bold text-base">
            {submitBulkAttendance.isPending ? "Submitting..." : "Submit Attendance"}
          </Text>
        </TouchableOpacity>
      </View>
    </ScreenWrapper>
