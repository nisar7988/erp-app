...
export default function AttendanceScreen() {
  ...
  const { data: records = [], isLoading: isRecordsLoading } = useAttendance(profile?.id);
  ...
  const overallPercentage = records.length > 0 ? Math.round((presentCount / records.length) * 100) : 0;
    ...
    return (
      <ScreenWrapper>
        <View className="flex-1 items-center justify-center">
          ...
          <Text className="mt-4 text-on-surface">Fetching attendance records...</Text>
        </View>
      </ScreenWrapper>
    );
  }
    ...
    <ScreenWrapper padding={false}>
      ...
      <Card variant="primary" className="mx-6 mt-4 p-8 items-center border-primary/10 bg-primary/5">
        <View className="relative items-center justify-center w-32 h-32 mb-6">
          <View className="w-28 h-28 rounded-full border-8 border-outline items-center justify-center">
            ...
            <View className="items-center">
              <Text className="text-3xl font-bold text-on-surface">{overallPercentage}%</Text>
        ...
        <Text className="text-xl font-bold text-on-surface text-center">
          {overallPercentage >= 90 ? "Excellent Attendance" : overallPercentage >= 75 ? "Good Attendance" : "Needs Improvement"}
        </Text>
      ...
      <View className="mt-10 px-6">
        <Text className="text-[10px] font-bold text-on-surface-variant tracking-widest mb-6 uppercase">
          {activeFilter === "All Sessions" ? "Recent Records" : `${activeFilter} Sessions`}
        </Text>
        ...
        <View className="pb-10">
          {filteredRecords.length > 0 ? (
            ...
            <Text className="text-sm text-on-surface-variant text-center mt-10">No records found for this filter.</Text>
          )}
        </View>
      </View>
    </ScreenWrapper>
